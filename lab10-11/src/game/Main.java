		...
		while(true) {
			...
			case "add":
				...
				try {
					...
					World.getInstance().getParty().add(HeroFactory.getInstance().createHero(heroType, cmdTokens[2]));
			...
			case "start":
				...
				if (!world.isAdventureStarted()) {
                    ...
                    World.getInstance().addObserver(TreasureDiscoverer.getInstance());
                    World.getInstance().addObserver(WorldPrinter.getInstance());
				}
				break;
